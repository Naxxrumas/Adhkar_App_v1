...
export const aggregateUserProgress = functions.firestore.onDocumentWritten(
    ...
    async (event) => {
        ...
        const { groupId, date: dateStr, userId, value } = logData as any;
        if (!groupId || !dateStr || !userId) return;
...
export const hourlyGracePeriodCheck = functions.scheduler.onSchedule('0 * * * *', async (event) => {
